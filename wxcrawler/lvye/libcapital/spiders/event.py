...
# pattern example : http://www.lvye.cn/huodong/beijing-----------list-1--beijing-0/
def getUrls():
    urls = []
    baseUrl = 'http://www.lvye.cn/huodong/beijing-----------list-'
    for category in range(1,10):
        url1 = baseUrl + str(category) + '--beijing-0/'
        urls.append(url1) 
    #end for
...
class EventSpider(CrawlSpider):
    name = "event"
    allowed_domains = ["www.lvye.cn"]
    start_urls = getUrls()
    rules = ()
    targetpattern = r'http://www.lvye.cn/trip/\d+'
    baseurl = 'http://www.lvye.cn/huodong/'
    ...
    def parse(self, response):
        ...
        try:
            ...
            links = hxs.select('//div[@class="act-title"]/a/@href')
            if not links:
                return
            #end if
            for link in links:
                ...
                if not v_url:
                    ...
                    validlink = 'http://www.lvye.cn' + v_url;
                    if isTargetPage(self.targetpattern,validlink):
                        yield Request( url=validlink, callback=self.parse_details )
            #end for
    ...
    def parse_details(self, response):
        print "[weixiao] Start scrapping Detailed Info...." + response.request.url
        try:
            title = response.xpath('//div[@class="zyjb_title"]/h2/text()').extract()[0].strip()
            #print title
            ...
            place = response.xpath('/html/body/div[2]/div[1]/div[1]/div[2]/div[2]/div/div[2]/dl[3]/dd/text()').extract()[0].strip()

            desc = stripHTMLTags(response.xpath('//div[@class="intro-info_box"]').extract()[0])
            
            imageurl = response.xpath('//div[2]/div[1]/div[1]/div[2]/div[1]/div/ul/li/a/img/@src').extract()[0].strip()
            #print imageurl
