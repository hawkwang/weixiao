...
class EventSpider(CrawlSpider):
    name = "event"
    allowed_domains = ["tiandaoedu.com"]
    start_urls = [
        'http://tiandaoedu.com/about/tdgg/list_1.html', 
        'http://tiandaoedu.com/about/tdgg/list_2.html', 
        'http://tiandaoedu.com/about/tdgg/list_3.html']

    attributes={}
    
    rules = ()
    targetpattern = r'http://tiandaoedu.com/about/tdgg/\d+.html'
    ...
    def parse(self, response):
        try:
            ...
            items = response.xpath('//div[@class="sty_one ofh"]')
	        ...
	        if(len(imageurl)==0):
                ...
                link = item.xpath('.//p[@class="ptit"]/a/@href').extract()[0]

                title = item.xpath('.//.//p[@class="ptit"]/a/text()').extract()[0]
	...
	desc = response.xpath('//div[@class="wzy_bot"]').extract()[0]
        desc = stripHTMLTags(desc)
        #print description


        eventdate = getDate(desc) 
        if(len(eventdate)==0):
            #guess year        
            submittime = response.xpath('//div[@class="wzlist ofh"]/p[2]/span[3]/text()').extract()[0]
            submittime = getDate(submittime)
            print submittime
            dateinfo = submittime.split('.')
            year = dateinfo[0]

            eventdate = year + '.' + getAnotherDate(desc) 
        #endif
        print eventdate

        eventtime = getTime(desc) 
        print eventtime
